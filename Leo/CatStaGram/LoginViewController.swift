...
//컨트롤러에서도 text값을 받아와야 로직을 처리할 수 있음
    var email = String()
    var password = String()
    ...
    @IBAction func emailTextFeildEdtingChanged(_ sender: UITextField) {
            let text = sender.text ?? ""
        self.loginButton.backgroundColor = text.isValidEmail() ? .facebookColor : .disabledButtoColor
            self.email = text
    ...
    @IBAction func passwordTextFeildEditingChanged(_ sender: UITextField) {
        ...
        = text.count > 2 ? .facebookColor : .disabledButtoColor
